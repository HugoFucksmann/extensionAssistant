...
export const getProjectSummary: ToolDefinition<typeof getProjectSummaryParamsSchema, ProjectSummaryData | null> = {
  ...
  async execute(
    ...
    try {
      ...
      const structure: TopLevelEntry[] = topLevelEntriesRaw.map(([name, fileTypeEnum]) => { // fileTypeEnum es vscode.FileType
        ...
        switch (fileTypeEnum) {
          ...
          case context.vscodeAPI.FileType.Directory:
            typeValue = 'directory';
            break;
          default: // Covers SymbolicLink, Unknown
            typeValue = 'other';
            break;
        }
      ...
      // Patrón para excluir directorios comunes y archivos de configuración/compilación grandes
      const excludePattern = '{**/node_modules/**,**/.*/**,**/.git/**,**/dist/**,**/build/**,**/coverage/**,**/*.log,**/yarn.lock,**/package-lock.json}';
      const filesForHeuristic = await context.vscodeAPI.workspace.findFiles('**/*.*', excludePattern, 150); // Escanear hasta 150 archivos

      filesForHeuristic.forEach(fileUri => {
        const ext = path.extname(fileUri.fsPath).toLowerCase();
        if (ext && ext.length > 1 && ext.length < 6) { // Extensiones razonables
          const lang = ext.substring(1);
          const commonLangMap: Record<string, string> = {
            tsx: 'typescript', jsx: 'javascript', pyw: 'python', htm: 'html',
            cxx: 'cpp', hxx: 'cpp', cc: 'cpp', hh: 'cpp', // Agrupar C++
