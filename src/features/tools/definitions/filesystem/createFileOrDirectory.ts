...
export const createFileOrDirectoryParamsSchema = z.preprocess(
...
export const createFileOrDirectory: ToolDefinition<typeof createFileOrDirectoryParamsSchema, CreateResultType> = {
  ...
  description: 'Creates a new file or a new directory with detailed metadata. The path must be provided in the "path" parameter. Creates parent directories if they do not exist. Overwrites existing files if type is "file".',
  ...
  async execute(params, context): Promise<ToolResult<CreateResultType>> {
    ...
    try {
      ...
      } catch (e) { /* Does not exist, will be created */ }
      ...
      if (type === 'directory') {
        if (existingStat) {
          if (existingStat.type === context.vscodeAPI.FileType.Directory) {
            ...
            return { success: false, error: `Path ${relativePath} exists and is not a directory.` };
          }
        }
        ...
        if (existingStat) {
          if (existingStat.type === context.vscodeAPI.FileType.Directory) {
            return { success: false, error: `Path ${relativePath} exists and is a directory. Cannot overwrite with a file.` };
          }
